...
#_______________________________________________________________________________________________________________________
#Start a video feed from computer webcam:

vidCap =  cv2.VideoCapture(0) #Selects the webcam as the camera to use - 0 for onboard webcam or 'http://JamesHall111:Stream67@192.168.0.129:8080/video' for IP webcam

print("Video Feed Start") #Notifies user when the capera feed is starting

while vidCap.isOpened():
    success, Image = vidCap.read()#If video capture is running the computer will read the next frame of said video

    if not success:
        print("Video Feed Stopped") #If the video feed is interrupted the program will notify the user then stop the program preventing errors
        break
...
#Implementing my YOLO26 trained model onto the new masked image
    ...
    cv2.imshow('Video Feed 2', annotated_frame)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break
